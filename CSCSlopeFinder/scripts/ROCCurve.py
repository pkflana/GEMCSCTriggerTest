...
plotdir = "plots/ROCCurve_HighestEMTFpT_ZeroBiasOnly/"
...
for evenodd in evenodd_list:
    ...
    for recopt_cut, emtfpt_cut in [[27,22]]:
        ...
        for ba_cut in range(50):
            bkg_num_cut = cut_bkg + f" & abs((-(LCT_eighthstrip-LCT_match_GE1_padES_aligned))) <= {ba_cut}"
            bkg_den_cut = cut_bkg

            sig_num_cut = cut_sig + f" & abs((-(LCT_eighthstrip-LCT_match_GE1_padES_aligned))) <= {ba_cut}"
            sig_den_cut = cut_sig
        ...
        canvas.SaveAs(plotdir+f"ROC_recopt{recopt_cut}_emtfpt{emtfpt_cut}_{evenodd}.pdf")
        ...
        canvas.SaveAs(plotdir+f"ROC_recopt{recopt_cut}_emtfpt{emtfpt_cut}_CSCGEMSlope_CSCGEM_{evenodd}.pdf")
        ...
        canvas.SaveAs(plotdir+f"ROC_recopt{recopt_cut}_emtfpt{emtfpt_cut}_CSCSlope_CSCGEM_{evenodd}.pdf")
        ...
        canvas.SaveAs(plotdir+f"ROC_recopt{recopt_cut}_emtfpt{emtfpt_cut}_CSCGEM_{evenodd}.pdf")
